    ...
    with connect_db() as conn:
        with conn.cursor() as cur:
            ...
            if group_data:
                ...
                cur.execute(
                    "SELECT login FROM users WHERE id IN (SELECT user_id FROM group_members WHERE group_id = %s)",
                    (group_.id,))
                ...
                return render_template('group.html', group=group_, group_members=group_members_,
                                       group_meetings=group_meetings_)
            else:
            ...
            with conn.cursor() as cur:
                ...
                if meeting_data:
                    ...
                    if group_data:
                        ...
                        return render_template('group.html', group=group_, group_members=group_members_,
                                               group_meetings=updated_meeting_data)
...
def create_group():
    if request.method == 'POST':
        ...
        with connect_db() as conn:
            with conn.cursor() as cur:
                ...
                if is_group_exists:
                    ...
                    cur.execute("INSERT INTO groups (name, password, type) VALUES (%s, %s, %s) RETURNING id",
                                (name_, password_, group_type_))
        ...
        print(
            f"Group created successfully! group_id: {group_.id}, name: {name_}, password: {password_}, group_type: {group_type_}")
        return render_template('group.html', group=group_, group_members=group_members_)
        ...
        with connect_db() as conn:
            with conn.cursor() as cur:
                ...
                if data:
                    ...
                    cur.execute("SELECT * FROM group_members WHERE group_id = %s AND user_id = %s",
                                (group_.id, user_id))
